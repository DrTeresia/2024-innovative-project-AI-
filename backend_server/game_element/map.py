...
File: map.py
Description: Defines the map class, which represents the map of the simulated
world in a 2-dimensional matrix.
"""
import json

from backend_server.global_methods import *
...
class Map:
    def __init__(self, map_name):
        ...
        # example variables, check out the map_meta_info.json file.
        meta_info = json.load(open(f"{env_matrix}/map_meta_info.json"))
        # <map_width> and <map_height> denote the number of tiles make up the
        # height and width of the map.
        self.map_width = int(meta_info["map_width"])
        self.map_height = int(meta_info["map_height"])
        ...
        map_folder = f"{env_matrix}/map"

        _cm = map_folder + "/collision_map.csv"
        collision_map_raw = read_file_to_list(_cm, header=False)[0]
        _sm = map_folder + "/sector_map.csv"
        sector_map_raw = read_file_to_list(_sm, header=False)[0]
        _am = map_folder + "/arena_map.csv"
        arena_map_raw = read_file_to_list(_am, header=False)[0]
        _gom = map_folder + "/game_object_map.csv"
        game_object_map_raw = read_file_to_list(_gom, header=False)[0]
        _slm = map_folder + "/spawning_location_map.csv"
        spawning_location_map_raw = read_file_to_list(_slm, header=False)[0]

        # Loading the map. The maps are taken directly from the json exports of
        # Tiled maps. They should be in csv format.
        # Importantly, they are "not" in a 2-d matrix format -- they are single
        # row matrices with the length of width x height of the map. So we need
        # to convert here.
        ...
        arena_map = []
        game_object_map = []
        spawning_location_map = []
        for i in range(0, len(collision_map_raw), meta_info["map_width"]):
            tw = meta_info["map_width"]
            self.collision_map += [collision_map_raw[i:i + tw]]
            sector_map += [sector_map_raw[i:i + tw]]
            arena_map += [arena_map_raw[i:i + tw]]
            game_object_map += [game_object_map_raw[i:i + tw]]
            spawning_location_map += [spawning_location_map_raw[i:i + tw]]

        # Once we are done loading in the map, we now set up self.tiles. This is
        ...
        for i in range(self.map_height):
            ...
            for j in range(self.map_width):
                ...
                tile_details["sector"] = ""
                if sector_map[i][j] in sb_dict:
                    tile_details["sector"] = sb_dict[sector_map[i][j]]

                tile_details["arena"] = ""
                if arena_map[i][j] in ab_dict:
                    tile_details["arena"] = ab_dict[arena_map[i][j]]

                tile_details["game_object"] = ""
                if game_object_map[i][j] in gob_dict:
                    tile_details["game_object"] = gob_dict[game_object_map[i][j]]

                tile_details["spawning_location"] = ""
                if spawning_location_map[i][j] in slb_dict:
                    tile_details["spawning_location"] = slb_dict[spawning_location_map[i][j]]
